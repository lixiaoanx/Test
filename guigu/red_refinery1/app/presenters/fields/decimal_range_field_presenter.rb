...
module Fields
  class DecimalRangeFieldPresenter < CompositeFieldPresenter
    ...
    def begin_options
      { min: min_begin, max: max_begin, disabled: begin_disabled?, step: step }.reject { |_, v| v.nil? }
    end

    def end_options
      { min: min_end, max: max_end, disabled: end_disabled?, step: step }.reject { |_, v| v.nil? }
    end
