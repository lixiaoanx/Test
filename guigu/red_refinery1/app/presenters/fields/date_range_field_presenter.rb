...
module Fields
  class DateRangeFieldPresenter < CompositeFieldPresenter
    def value_for_preview
      ...
      from =
        if record.begin && record.begin != -Float::INFINITY
          I18n.l(record.begin.in_time_zone.to_date)
        end
      to =
        if record.end && record.end != Float::INFINITY
          I18n.l(record.end.in_time_zone.to_date)
        end
